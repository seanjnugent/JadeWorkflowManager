...
# Define operations (ops) with unique names
@op(config_schema={"input_file_path": str}, out=Out(pd.DataFrame))
def load_input_op_2sfdsfgsd1(context):
...
def transform_op_2sfdsfgsd1(context, input_df: pd.DataFrame):
...
def save_output_op_2sfdsfgsd1(context, input_data: dict):
...
# Define the job with unique operation names
@job(
    name="workflow_job_2sfdsfgsd1",
    resource_defs={
        "supabase": supabase_resource,
        "db_engine": db_engine_resource
    }
)
def workflow_job_2sfdsfgsd1():
    raw_data = load_input_op_2sfdsfgsd1()
    transformed_data = transform_op_2sfdsfgsd1(raw_data)
    save_output_op_2sfdsfgsd1(transformed_data)
