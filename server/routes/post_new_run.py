...
def upload_to_storage(file_path: str, content: bytes, supabase_client: Client) -> str:
    """
...
def validate_file_structure(df: pd.DataFrame, expected_structure: Dict) -> None:
    ...
    Args:
        df: Input DataFrame
        expected_structure: Expected structure from workflow.input_structure
    """
    try:
...
def run_dagster_job(job_name: str, run_config: Dict) -> Dict:
    ...
    Args:
        job_name: Name of the Dagster job (e.g., workflow_job_22)
...
async def create_run(
    ...
    """
    Trigger a Dagster job run for a workflow with an uploaded file.
    ...
    try:
        ...
        storage_url = upload_to_storage(file_path, content, supabase)
        ...
        logger.info(f"Triggering Dagster job: {job_name}")
        try:
