    ...
    const StatusBadge = ({ status }) => {
      const statusMap = {
        ...
        completed: { color: 'bg-green-50 text-green-700 border-green-200', icon: <CheckCircle className="h-4 w-4" />, label: 'Completed' },
        ...
        cancelled: { color: 'bg-purple-50 text-purple-700 border-purple-200', icon: <XCircle className="h-4 w-4" />, label: 'Cancelled' }
      };
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="flex items-center gap-3">
                          <div className="flex-0 w-8 h-8 bg-gray-100 flex items-center justify-center border border-gray-200">
                            ...
                            <div key={step.key} className="relative pl-8">
                              ...
                              <div className="absolute left-1.5 top-4 w-4 h-4 border-2 border-gray-300 flex items-center justify-center">
                                <div className={`w-2 h-2 ${
                                  ...
                                  stepStatus.status?.toLowerCase() === 'completed' ? 'bg-green-600' :
                                  stepStatus.status?.toLowerCase() === 'failed' ? 'bg-red-600' :
                                  stepStatus.status?.toLowerCase() === 'failure' ? 'bg-red-600' :
                                  stepStatus.status?.toLowerCase() === 'skipped' ? 'bg-orange-500' :
                                  stepStatus.status?.toLowerCase() === 'cancelled' ? 'bg-purple-500' : 'bg-gray-400'
                                }`} />
                              </div>
